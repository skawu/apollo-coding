...
uint8_t SDRAM_Send_Cmd(uint8_t bankx, uint8_t cmd, uint8_t refresh, uint8_t regval)
{
	...
	Command.AutoRefreshNumber = refresh;			// 自动刷新次数
	Command.ModeRegisterDefinition = regval;		// 要写入模式寄存器的值
	
	if(HAL_SDRAM_SendCommand(&SDRAM_Handler, &Command, 0x1000) == HAL_OK)	// 向SDRAM发送数据
		return 0;
	else 
		return 1;
}
...
void SDRAM_Initialization_Sequence(SDRAM_HandleTypeDef *hsdram)
{
	...
	while(delay--);												// 延时至少200us，此处延时500us
	SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_PALL, 1, 0);				// 对所有存储区预充电
	SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_AUTOREFRESH_MODE, 8, 0);	// 设置自动刷新次数
	
	// 配置模式寄存器，SDRAM的bit0~bit2为指定突发访问长度
		// bit3为指定突发访问的类型，bit4~bit6为CAS值，bit7~bit8为运行模式
		// bit9为指定的写突发模式，bit10和bit11为保留位
	
	temp = (uint32_t)SDRAM_MODEREG_BURST_LENGTH_1			|		// 设置突发长度：1（可以是1/2/4/8）
					SDRAM_MODEREG_BURST_TYPE_SEQUENTIAL		|		// 设置突发类型：连续（可以是连续/交错）
					SDRAM_MODEREG_CAS_LATENCY_3				|		// 设置CAS值：3（可以是2/3）
					SDRAM_MODEREG_OPERATING_MODE_STANDARD 	|		// 设置操作模式：0，标准模式
					SDRAM_MODEREG_WRITEBURST_MODE_SINGLE;			// 设置SDRAM的模式寄存器
	SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_LOAD_MODE, 1, temp);
	
	// 刷新频率计数器(以SDCLK频率计数)，计算方法：
		// COUNT = SDRAM刷新周期 / 行数 - 20 = SDRAM刷新周期(us) * SDCLK频率(MHz) / 行数 - 20
	// 使用的SDRAM舒心周期为64ms，SDCLK = 180 / 2 = 90MHz,行数为8192(2^13)
...
void SDRAM_Init(void)
{
	...
	SDRAM_Handler.Init.SDBank = FMC_SDRAM_BANK1;							// SDRAM在BANK5
	...
	SDRAM_Handler.Init.MemoryDataWidth = FMC_SDRAM_MEM_BUS_WIDTH_16;		// 数据宽度为16bit
	SDRAM_Handler.Init.InternalBankNumber = FMC_SDRAM_INTERN_BANKS_NUM_4;	// 内部共有4个Bank
	SDRAM_Handler.Init.CASLatency = FMC_SDRAM_CAS_LATENCY_3;				// CAS 列地址选通信号，3个
	...
	SDRAM_Timing.LoadToActiveDelay = 2;										// 加载模式寄存器到激活时间的延时为两个时钟周期
	SDRAM_Timing.ExitSelfRefreshDelay = 8;									// 退出自动刷新延迟为8个时钟周期
	SDRAM_Timing.SelfRefreshTime = 6;										// 自刷新时间为6个时钟周期
	SDRAM_Timing.RowCycleDelay = 6;											// 行循环延迟为6个时钟周期
	SDRAM_Timing.WriteRecoveryTime = 2;										// 恢复延时为2个时钟周期
